...
class ExpressionTree:
    ...
    def __build_tree(self):
        postfix = self.__infix_to_postfix(self.__tokens)  # converte a expressÃ£o infixa para pÃ³s-fixada
        stack = []  # pilha para armazenar os nÃ³s da Ã¡rvore
        for token in postfix:  # percorre os tokens da expressÃ£o pÃ³s-fixada
            if not self.__is_operator(token):  # se o token nÃ£o Ã© um operador
                stack.append(BinaryNode(token))  # adiciona o token como um nÃ³ folha na pilha
            else:  # se o token Ã© um operador
                right = stack.pop()  # remove o nÃ³ da direita da pilha
                left = stack.pop()  # remove o nÃ³ da esquerda da pilha
                node = BinaryNode(token)  # cria um novo nÃ³ com o operador
                node.set_left_node(left)  # define o nÃ³ da esquerda
                left.set_parent(node)  # define o pai do nÃ³ da esquerda
                node.set_right_node(right)  # define o nÃ³ da direita
                right.set_parent(node)  # define o pai do nÃ³ da direita
                stack.append(node)  # adiciona o novo nÃ³ Ã  pilha
        self.__root = stack[0] if stack else None  # define a raiz da Ã¡rvore como o Ãºnico nÃ³ restante na pilha

    def __infix_to_postfix(self, tokens):
        precedence = {'**': 4, '*': 3, '/': 3, '+': 2, '-': 2}  # define a precedÃªncia dos operadores
        right_assoc = {'**'}  # define os operadores com associatividade Ã  direita
        output = []  # guarda a saÃ­da em notaÃ§Ã£o pÃ³s-fixada
        stack = []  # guarda operadores e parÃªnteses
        for token in tokens:  # percorre os tokens da expressÃ£o
            if not self.__is_operator(token) and token not in ('(', ')'):  # se o token nÃ£o Ã© um operador ou parÃªnteses
                output.append(token)
            elif token in precedence:  # se o token Ã© um operador
                while (stack and stack[-1] in precedence and  # verifica se o topo da pilha Ã© um operador
                       ((token not in right_assoc and precedence[token] <= precedence[
                           stack[-1]]) or  # verifica a precedÃªncia
                        (token in right_assoc and precedence[token] < precedence[
                            stack[-1]]))):  # verifica a precedÃªncia
                    output.append(stack.pop())  # adiciona o operador Ã  saÃ­da
                stack.append(token)  # adiciona o operador Ã  pilha
            elif token == '(':  # se o token Ã© um parÃªntese esquerdo
                stack.append(token)  #
            elif token == ')':  # se o token Ã© um parÃªntese direito
                while stack and stack[
                    -1] != '(':  # enquanto a pilha nÃ£o estiver vazia e o topo nÃ£o for um parÃªntese esquerdo
                    output.append(stack.pop())  # adiciona os operadores Ã  saÃ­da
                stack.pop()  # Remove '('
        while stack:  # enquanto a pilha nÃ£o estiver vazia
            output.append(stack.pop())  # Adiciona os operadores restantes Ã  saÃ­da
        return output  # retorna a saÃ­da em notaÃ§Ã£o pÃ³s-fixada

    # def __build_tree(self):
    #     root = None
    #     for i in range(len(self.__tokens)):
    #         if self.__is_operator(self.__tokens[i]):  # Ã© um operador
    #             if not root:
